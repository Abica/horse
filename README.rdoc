...
to be run before the next repaint is occurs. Usually RAF matches your display's refresh rate, however Horse
...
is ever running at a time, and it sets up a single RAF run loop which will iterate through any added jobs and step
